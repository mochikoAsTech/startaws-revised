...
クラウドだけではありません。よくクラウドと一緒に並んでいる「サーバ」や「インフラ」という言葉がありますが、こちらも何だか分かりますか？IT系で働いていても、その辺って「なんか…ふんわり…なんか雲の向こう側にある…ウェブサイト作るための何か…？」という程度の認識で、「クラウドってなに？」とか、「サーバってなに？」と聞かれたときに、ちゃんと説明できる人は意外と少ないのではと思います。

なので、先ずは「@<ttb>{AWSはAmazonがやっているクラウド}」という文章の意味が分かるよう、インフラ周りから順を追って学んでいきましょう。
...
これでもう会社で後輩に「インフラってなんですか？」と聞かれても、堂々と「サーバとかネットワークのことだよ」と答えられますね！でも後輩に、続けて「え、サーバってなんですか？」と聞かれたらどうしましょう？

== サーバとは？

IT系、その中でも特にウェブ業界で働いている人にとってサーバは身近な存在です。業務上でテストサーバや本番サーバ、ウェブサーバやデータベースサーバなどの単語を見聞きすることはよくあると思います。また日常生活でも「テレビで紹介されたらアクセスが殺到してサーバが落ちたんだって」とか「ソシャゲのアプリで遊ぼうとしたのにサーバが重くてなかなか遊べない」というように、「サーバ」という単語は普通に使われています。

でももし後輩から「サーバってなんですか？」という直球の質問を投げつけられたら、しっかりホームランで打ち返せますか？サーバっていったい何なのでしょう？
...
//image[imagineServer][サーバの姿を実写で思い浮かべて描いてみよう][scale=0.5]{
//}
...
//image[pcImage][パソコンの姿ならすぐに思い浮かぶ][scale=0.6]{
//}
...
//footnote[dl360][HPE ProLiant DL360。画像は@<href>{https://www.hpe.com/jp/ja/product-catalog/servers/proliant-servers/pip.hpe-proliant-dl360-gen10-server.1010007891.html}より引用。]
//footnote[gen10][ちなみに2020年12月時点で販売されているのはHPE ProLiant DL360 Gen10です。末尾の「Gen10」は世代（Generation）を表しているので、10世代目ということです。]
...
//footnote[HPEAdvancedG2][このサーバラックはHPE Advanced G2シリーズ。画像は@<href>{https://www.hpe.com/jp/ja/product-catalog/servers/server-racks/pip.hpe-g2-advanced-series-racks.1009694916.html}より引用。]
...
//footnote[42u][但しラックに供給される電源の量や放熱の問題もあるため、実際は42Uサイズのラックにサーバ42台をぎちぎちに詰められるとは限りません。]
...
//footnote[tower][画像は@<href>{https://www.hpe.com/jp/ja/product-catalog/servers/proliant-servers/pip.hpe-proliant-ml110-gen10-server.1010192782.html}より引用。]
//footnote[blade][画像は@<href>{https://www.hpe.com/jp/ja/integrated-systems/bladesystem.html}より引用。]
...
//footnote[serverRoom][企業によっては、オフィス内にサーバルームがあってサーバはそこに設置されているかも知れません。趣味で自宅にサーバを置いている人もいますね。排熱のお陰で冬場でも暖房要らずだ、と聞いたことがあります。]
...
もし悪い誰かが「この企業が気に食わないから商品のウェブサイトを落としてやろう」と思ったとき、あるいは「このネットショップの顧客情報を根こそぎ盗んでやろう」と思ったとき、ネット越しにサイトを攻撃したり侵入したりするだけでなく、そのサイトが動いているサーバのところまで行って物理的に破壊したり、ハードディスクを引っこ抜いて盗んだりする、という手段があります。
...
入退出時の体重チェックは健康を心配してくれているわけではなく、盗んだハードディスクを持ち出せないようにするためです。
...
電源がなくなればパソコンも落ちてしまうように、サーバもその上で稼動しているウェブサイトも電力が供給されなければ落ちてしまいます。仮にサーバを2台用意して「1台壊れても、もう1台があれば大丈夫！」という冗長構成にしていても、データセンターの電力そのものが止まってしまえば、どちらのサーバも電源が切れてサイトは見られなくなります。電気は使えて当たり前と思いがちですが、東日本大震災後の輪番停電のように「当たり前が崩れたとき」にもいつもどおり稼動できる環境がデータセンターには求められているのです。@<fn>{hokkaido}

//footnote[hokkaido][2018年9月に北海道の胆振地方で震度7の地震が発生して道内のほぼ全域が停電してしまったとき、さくらインターネットの石狩データセンターはUPSと自家発電で稼動を続けました。当時、筆者が使っていた「さくらのVPS」というサーバも石狩DCにありましたが、止まることなく動き続けていて、普段から「こういうとき」のために備えていたんだなと涙が出そうな気持ちになりました。一部電源切替がうまくいかなかったUPSが原因で数時間停止したサービスもあったそうですが、それでも事業と地震の規模を考えると本当に頭が下がります。ありがとう、さくらインターネット！]
...
ところで部屋を借りるときは「2LDKの部屋なら12万円、3LDKの部屋なら20万円」のように広いほうが家賃は高くなりますよね。データセンターを借りるときもまったく同じで「ラックの1/2なら12万円、1ラックまるごとなら20万円」のように、ラックサイズによって月額の費用が変わってきます。

そして3LDKの部屋に1人で住むと、1人で20万円負担しなければなりませんが、シェアハウスにして2人で住めば1人当たりの家賃コストは10万円に下がります。サーバラックも同様で、42Uのラックに1Uサーバを1台しか乗せないより42台詰め込んだ方が1台当たりのラック使用コストは下がります。
...
//footnote[power][サーバラックへサーバをぎちぎちに詰め込んでなんとか稼動していたものの、ある日データセンターで停電が起きて全台停止。電源はすぐに復旧したため、自動で全台が一斉に起動しようとしたが、サーバは起動時がいちばん電源を食うためラックのブレーカーが落ちて再度全台停止。一斉に起動しようとしてはブレーカーが落ちて全台停止、を延々と繰り返していた…という怪談を聞いたことがあります。本当に怖い話ですね。]
...
設置できたら、今度は同じく自前のネットワーク機器からLANケーブルを繋ぎ、電源も繋ぎます。そしてOSのインストールディスクを用意して、サーバにOSをインストールして…以下省略しますが、要は「@<ttb>{ただ自社のウェブサイトが作りたいだけなのにサーバを用意するまでがすごく大変だった}」ということです。
...
さらに、もしピザ屋が大繁盛したら、フードコート内で自店の隣の区画も借りてお店を広くすることも簡単にできるので、初めから広い区画を借りておく必要もありません。つまり、ウェブサイトへのアクセスが増えてきてサーバのスペックが足りなくなったら、@<ttb>{後からサーバを増強したり好きなだけサーバの台数を増やしたりもできる}ので、最初から高スペックなサーバを借りておく必要がないということです。クラウドなら初期投資額が少なく、すぐにはじめられてすぐにやめられる。よく「クラウドはスモールスタートに向いている」と言われますが、その理由はまさにこういうところにあるのです。
...
またサーバを購入して所有していれば会社の資産となりますが、クラウドの場合はどれだけ長く使ってもサーバは自社のものにはなりません。あくまで借りているだけです。会計の視点から見るとオンプレミスの場合はサーバ代は固定費となるため先々の見通しもつけやすいですが、クラウドの場合は使った分だけの変動費となるため費用の予測はあくまで予測となります。さらに長い目で見るとフードコートにテナント料を払い続ける方が、土地や建物を買うよりも最終的には高くなるかも知れません。前述のとおり初期投資は少なくて済むのですが、クラウドのいいところは決して「コストが安くなる」ということではありません。実際、AWSは他社の共有レンタルサーバやVPS@<fn>{vps}と単純比較すると高額です。

//footnote[vps][Virtual Private Serverの略。先ほど出てきた仮想サーバのことだと思ってください。]

クラウドのよいところは、前述のすぐに始められてすぐにやめられる@<ttb>{初期コストの低さ}。それからショッピングセンター内でフードコートが入っている南館が火災で倒壊してしまっても、すぐに北館に移ってピザ屋の営業を再開できる、といった@<ttb>{可用性}です。

この可用性を自力で確保しようとしたら大変です。ピザ屋を常に営業し続けておくために、いつ来るか分からない火災や天変地異に備えて、最初から予備の店舗も確保しておかなければならないとしたら相当なコストがかかります。オンプレミスに当てはめて考えると、ただ自社サイトを作りたいだけなのに、品川と渋谷の2か所でデータセンターを借りて両方に1台ずつサーバを用意しておき、もし品川のデータセンターが火災で使えなくなっても渋谷のデータセンターにあるサーバは生きているのでサイトは見られる、というような構成にしておかなければならない、ということです。予見できない災害に対してどこまで冗長性を確保@<fn>{dr}しておくのか？はシステムの重要性とコストのバランスによります。このような構成が手間も費用も低コストで実現できるクラウドはすごいですよね。
...
もし何かトラブルがあってフードコート全体がお休みになるときは、問答無用でピザ屋もお休みになってしまいます。つまり使っているクラウドで大規模障害が起きたら、一利用者である私たちにできることはなく、復旧までひたすら待つしかない、ということです。AWSでも広範囲にわたる障害はたびたび起きています。たとえば2016年6月には豪雨による電源障害でサーバに接続できなくなる事象が発生@<fn>{awsTrouble201606}しました。2019年8月には東京のデータセンターで空調設備に問題があり、室温が上がったことでサーバが止まってしまう障害が発生しました。このときはPayPayで支払いができなくなったり、Huluが見られなくなったり、ユニクロのオンラインサイトで買い物ができなくなったりと実生活にさまざまな影響@<fn>{awsTrouble201908}が出ていました。最近では2020年11月にバージニア州のデータセンターで障害が起き@<fn>{awsTrouble202011}、エアコンや照明を操作するスマートリモコンの「Nature Remo」がこの障害の影響を受けて動作しなくなる、という出来事もありました。こうした障害の際もAWSが発表してくれる内容がすべてですので、原因が分かるまで自分で徹底的に調べる、あるいは自力で何とかする、ということはできません。

//footnote[awsTrouble201606][Amazonクラウドのシドニーリージョン、豪雨による電源障害でEC2などに一部障害。現在は復旧 － Publickey @<href>{https://www.publickey1.jp/blog/16/amazonec2.html}]
//footnote[awsTrouble201908][AWS 東京リージョンで発生した大規模障害についてまとめてみた - piyolog @<href>{https://piyolog.hatenadiary.jp/entry/2019/08/23/174801}]
//footnote[awsTrouble202011][AWSで障害、「Nature Remo」「SwitchBot」などに影響　「電気消せない」と嘆く声【追記あり】 - ITmedia NEWS @<href>{https://www.itmedia.co.jp/news/articles/2011/26/news056.html}]
...
//footnote[sameCloud][他店舗というのは、具体的には同じホストサーバを使っている他のゲストサーバ上のサイト。あるいは同じインターネット回線を使っている他のサーバ上のサイトのことです。]
...
ところでクラウドはAWSだけではありません。GoogleのGoogle Cloud Platform@<fn>{gcp}、MicrosoftのAzure（アジュール）@<fn>{azure}、オラクルのOracle Cloud@<fn>{oraclecloud}、中国のAlibaba Cloud@<fn>{alibaba}やTencent Cloud@<fn>{tencent}、国内だとさくらインターネットがやっているさくらのクラウド@<fn>{sakura}など、たくさんのクラウドが存在しています。

//footnote[gcp][@<href>{https://cloud.google.com/}]
//footnote[azure][@<href>{https://azure.microsoft.com/ja-jp/}]
//footnote[oraclecloud][@<href>{https://www.oracle.com/jp/cloud/}]
//footnote[alibaba][@<href>{https://www.alibabacloud.com/ja}]
//footnote[tencent][@<href>{https://intl.cloud.tencent.com/}]
//footnote[sakura][@<href>{https://cloud.sakura.ad.jp/}]

その中でもなぜ「AWSがいい」と言われているのでしょうか？理由は使う人によってそれぞれだと思いますが、私なりの「なぜAWSなのか？」を考えてみました。

2020年8月の調査@<fn>{cloudShare}によると、クラウド市場ではAWSがシェア約45%でトップを独走中です。そのため他のクラウドと比べると、AWSなら使ったことがあって対応可能だ、というエンジニアも多く、何か困ったときに調べて出てくる情報も多い、というのが、私がAWSを選ぶいちばんの理由です。それ以外だと、利益が出た分だけどんどん投資されてサービスが改良されていくため、細かな使い勝手がどんどん良くなっていく@<fn>{improve}、というところもポイントです。

//footnote[cloudShare][IaaS型クラウドにおけるAWSのシェアは45％、Azureが18％で2位、3位はアリババ。2020年8月のガートナー調査 － Publickey @<href>{https://www.publickey1.jp/blog/20/iaasaws45azure182320208.html}]
//footnote[improve][画面や機能もどんどん変わっていくので、この後出てくる設定画面も、皆さんが手を動かしてやってみる頃には本書のキャプチャとは違うものになっているかも知れません。AWSのいいところでもあり、本やマニュアルを作って説明する側にとってはつらいところでもあります。]

しかし最近は、Alibaba CloudやOracle Cloud@<fn>{alwaysFreePlan}といった後発のクラウド事業者も追い上げを見せています。こうした新興のクラウドは、先を行くAWSを見て学んだ上で生まれてきただけあって、よりスマートな作りになっているのがいいところです。

//footnote[alwaysFreePlan][Oracle Cloudには、なんとサーバを有効期限なしでずっと無料で使えるAlways Freeという枠があります。本書の続編である「SSLをはじめよう」では、このAlways Freeを使ってHTTPSのサイトを作ります。　@<href>{https://www.oracle.com/jp/cloud/free/}]

クラウドを選ぶ理由、その中でもAWSを選ぶ理由というのは、普遍的な何かがあるわけではなく、本来であれば使う人やその上で動かすサービスの性質によって異なるはずです。あなたが動かしたいサービスによっては、AWSではなく他のVPSやオンプレミスの方がいいケースだってもちろんあるはずです。これから使ってみて、あなた自身がAWSを選ぶ理由をちゃんと発見できたらいいですね。
