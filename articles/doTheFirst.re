...
//image[startDNS_v2][「DNSをはじめよう」（1,000円）はBOOTHで好評発売中][scale=0.3]{
//}
...
//image[awsUnauthorizedUse][「AWS 不正利用 請求」で検索すると不正利用による請求の体験記がたくさん][scale=0.6]{
//}
...
//image[mfaAppli01][「Google認証システム」のアプリを起動したら右下の＋ボタンをタッチ][scale=0.2]{
//}

//footnote[quality][「Google認証システム」は画面のキャプチャできないようになっていたため、スマホ画面をカメラで撮るという暴挙に出ました。ここだけ画質が低くてごめんなさい。]

//image[mfaAppli02][「QRコードをスキャン」を選択][scale=0.2]{
//}
...
//image[mfaAppli04][「start-aws-user」用の認証コード（6桁の数字）が表示されるようになる][scale=0.2]{
//}
...
//image[mfaAppli05][認証コードは30秒ごとに次々と切り替わる][scale=0.2]{
//}
...
右上の「コンソールへサインイン」@<fn>{signinOrLogin}をクリック（@<img>{signInToConsole}）します。

//footnote[signinOrLogin][ちなみに右上には「コンソールにサインイン」とありますが、画面中央には「もう一度ログインする」と書いてあります。このサインインとログインのような表記揺れば、特に日本語のマネジメントコンソールではよくあることです。日本語の翻訳がいまいちなせいで逆に分かりにくくなっているところもあるので、マネジメントコンソールの言語を英語にした方がいっそ分かりやすいかも知れません。英語が苦手な方は随時脳内補完しながら頑張りましょう。]
...
//image[mfaCode][スマホで「Google認証システム」を起動][scale=0.2]{
//}
...
いったん「start-aws-user」でサインアウトしたら、今度はルートユーザーでマネジメントコンソール@<fn>{consoleThreeTimes}にサインイン（@<img>{rootLogin}）しなおしてください。

//footnote[consoleThreeTimes][@<href>{https://console.aws.amazon.com/}]

//image[rootLogin][ルートユーザーでマネジメントコンソールにサインインしなおす][scale=0.8]{
//}

ルートユーザーでサインインできたら、マネジメントコンソールの左上にある「サービス」を開いて、「セキュリティ、ID、およびコンプライアンス」の下にある「IAM」（@<img>{iam}）をクリックし、IAMのダッシュボードを開きます。

//image[accessService2][サービス＞セキュリティ、ID、およびコンプライアンス＞IAM][scale=0.8]{
//}

IAMのダッシュボードで「MFAを有効」をクリック（@<img>{rootAccountMfa}）します。

//image[rootAccountMfa][「MFAを有効」をクリック][scale=0.8]{
//}
...
//image[mfaCodeForRoot][IAMユーザーだけでなくルートアカウント用の認証コード（6桁の数字）も表示されている][scale=0.2]{
//}

ルートアカウントでもMFAが有効になったら、再びIAMユーザーの「start-aws-user」でサインインしなおして先へ進みましょう。もしどこのURLからサインインするのか分からなくなってしまったら、先ほど送っておいた「アマゾン ウェブ サービスへようこそ」という件名のメールに、「サインイン URL: @<href>{https://602837347040.signin.aws.amazon.com/console}」（数字の部分は人によって異なります）というURLがあるはずですので、そこをクリックしてサインインしましょう。MFAコードを聞かれたらスマホのGoogle認証システムを起動して、表示されている6桁の数字を入力します。
...
マネジメントコンソールの右上（@<img>{signInAgainAndAgain}）に表示されているユーザー名が「start-aws-user」であること、そしてその右側に表示されているリージョンが「東京」であることを確認してください。ここがもし「東京」以外になっていたら、クリックして「アジアパシフィック(東京) ap-northeast-1」を選択（@<img>{selectRegion}）してください。選択後、右上が「東京」に変わったらリージョンの変更は完了です。

//image[selectRegion][「東京」以外のときはクリックして「アジアパシフィック(東京)」を選択][scale=0.5]{
//}

AWSはバージニア、カナダ、ロンドン、シンガポール、東京など世界の各地域にデータセンターを所有しており、@<chapref>{infraAndServer}で詳しくお話ししたようにサーバはそのデータセンターの中にいます。

右上に表示されている@<ttb>{「リージョン」とはその各地域の中でどこを使うか？を指定するもの}です。ウェブサイトにアクセスするとき、パソコンのある場所からサーバまで物理的に距離が遠いと、それだけ通信にも時間がかかって応答時間も遅くなりますので、日本国内向けにウェブサイトを開設する場合は基本的にこの「東京」リージョンを選びましょう。ただしAWSのサービスによってはまだ東京リージョンが使えないものもあります。その場合は次点として「米国東部 (バージニア北部) us-east-1」を選択してください。@<fn>{northVirginia}
...
またリージョンという地域ごとの区分の下に、さらにアベイラビリティゾーン@<fn>{az}という区分があります。アベイラビリティゾーンの場所は公開されていませんが、たとえば東京リージョンの下に「池袋アベイラビリティゾーン」と「品川アベイラビリティゾーン」がある、というようなイメージです。
...
リージョンの確認が済んだら、続いてCloudTrail（クラウドトレイル）の設定へ進みましょう。マネジメントコンソールの左上にある「サービス」から、「管理とガバナンス」の下にある「CloudTrail」（@<img>{selectCloudTrail}）をクリックしてください。

//image[selectCloudTrail][サービス＞管理とガバナンス＞CloudTrail][scale=0.8]{
//}

「CloudTrail」をクリックすると、CloudTrailのダッシュボード（@<img>{cloudTrailDashboard}）が表示されます。CloudTrailはAWSのマネジメントコンソールや、プログラムを通じて行われたAPI呼び出しの履歴を保存してくれるサービス…と書くと難しいですが、要は@<ttb>{「いつ誰が何をしたのか」を記録しておいてくれる}サービスです。先ずは左メニューを開きましょう。

//image[cloudTrailDashboard][CloudTrailダッシュボードで左メニューを開く][scale=0.8]{
//}

CloudTrailダッシュボードの左メニューにある「イベント履歴」をクリック（@<img>{cloudTrailDashboard2}）して、イベント履歴を確認してみましょう。

//image[cloudTrailDashboard2][「イベント履歴」をクリック][scale=0.8]{
//}

CloudTrailはデフォルトで有効になっているため、今まで行ってきた「サインイン」や「IAMユーザーの作成」などもすべてイベントとして記録されています。たとえばフィルターを「ユーザー名」にして「start-aws-user」で検索（@<img>{searchCloudTrail}）してみると、いつMFA（多要素認証）を有効にしたのか、なども確認できます。@<fn>{utc9}

//image[searchCloudTrail][start-aws-userがいつ何をしたのかすべて表示される][scale=0.8]{
//}
//footnote[utc9][表示されているイベント時間はUTC+9ですので日本時間ですね。]
