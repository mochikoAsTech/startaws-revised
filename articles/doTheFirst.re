...
AWSアカウントの作成は「DNSをはじめよう」（@<img>{startDNS}）@<fn>{booth}の「第3章 AWSのネームサーバ（Route53）を使ってみよう」で済ませていますので、本書でもそのAWSアカウントを引き続き使用していきます。
...
アラームの作成画面（@<img>{credits06}）で「超過」の欄に1USDと書くと、今月の利用料が1ドルを超えた時点でアラートメールが来ます。本書の内容でAWSを利用した場合、無料利用枠からはみ出す分は最大でも月2ドル程度ですのでここには2と書いておきます。「通知の送信先」には自分のメールアドレスを記入してください。
...
先ずは手順1の「グループ名」です。本書ではIAMのグループは「start-aws-group」にします。グループ名の欄に「start-aws-group」と入力して、右下の「次のステップ」をクリック（@<img>{addGroupStep1}）してください。
...
先ずはステップ1（@<img>{iamStep1}）です。IAMのユーザー名を入力してください。本書ではIAMのユーザ名は「start-aws-user」にします。IAMのユーザー名はこの後もサインイン時に使用しますので、もし別のユーザー名にした場合は、忘れないようしっかりメモしておいてください。
...
の2つがあります。本書ではマネジメントコンソールからしか操作しないため、「アクセスの種類」は「AWSマネジメントコンソールへのアクセス」にのみチェックを入れてください。
